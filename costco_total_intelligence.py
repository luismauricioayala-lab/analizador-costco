...
st.set_page_config(
    page_title="COST Institutional Master Terminal v18.0",
    page_icon="🏛️",
    layout="wide",
...
class ValuationModel:
    ...
    @staticmethod
    def run_dcf_pro(fcf, g1, g2, wacc, tg=0.025, shares=443.6, cash=22.0, debt=9.0, macro_adj=0.0):
        """Modelo DCF de dos etapas con ajuste de valor de capital y macro shock."""
        # Ajuste macro inicial sobre el FCF base
        base_fcf = fcf * (1 + macro_adj)
        projs = []
        df_flows = []
        curr = base_fcf
...
def main():
    ...
    # 2. Sidebar: Panel de Auditoría Maestra (Macro & Valuación)
    ...
    fed_funds = st.sidebar.slider("Tasas Fed Funds (%)", 0.0, 10.0, 4.25) / 100
    
    st.sidebar.markdown("### Blended GDP (Por Geografías)")
    gdp_us = st.sidebar.slider("PIB EE.UU (%)", -4.0, 8.0, 2.3) / 100
    gdp_intl = st.sidebar.slider("PIB Internacional (%)", -4.0, 8.0, 3.1) / 100
    # Peso Costco: 75% USA, 25% International
    blended_gdp = (gdp_us * 0.75) + (gdp_intl * 0.25)

    # --- LÓGICA DE SENSIBILIDAD MACRO ---
    # La inflación reduce márgenes (-1.2x). El ingreso disponible aumenta ticket (+1.5x).
    # La Fed afecta el WACC final directamente.
    macro_fcf_impact = (income_g * 1.5) + (blended_gdp * 0.8) - (inflation * 1.2) - ((u_rate - 3.5) * 0.03)
    final_wacc = wacc_base + (fed_funds / 12) # Ajuste de riesgo por liquidez de mercado

    # 3. Cálculos de Valoración con Impacto Macro
    f_val, flows, pv_f, pv_t = ValuationModel.run_dcf_pro(
        data['fcf_now_b'], g1_in, g2_in, final_wacc, 
        shares=data['shares_m'], cash=data['cash_b'], debt=data['debt_b'],
        macro_adj=macro_fcf_impact
    )
    upside = (f_val / p_ref - 1) * 100

    # 4. Renderizado de Cabecera con Coloración Lógica para Beta
    st.title(f"🏛️ {data['info'].get('longName')} Master Terminal")
    st.caption(f"Sync SEC 2026 | Alpha v18.0 | Macro-Adjusted WACC: {final_wacc*100:.2f}%")

    m1, m2, m3, m4 = st.columns(4)
    m1.metric("P/E TTM", f"{data['info'].get('trailingPE', 52.9):.1f}x", "Premium")
    m2.metric("Mkt Cap", f"${data['mkt_cap_b']:.1f}B", "NASDAQ: COST")
    
    # LÓGICA DE COLOR NEUTRO PARA BETA
    beta_val = data['beta']
    if 0.95 <= beta_val <= 1.05:
    ...
    m3.metric("Riesgo Beta", f"{beta_val:.3f}", b_label, delta_color=b_color)
    m4.metric("Intrinsic Value", f"${f_val:.2f}", f"{upside:+.1f}%", delta_color="normal" if upside > 0 else "inverse")

    st.markdown("---")

    # 5. ARQUITECTURA DE 10 PESTAÑAS
    tabs = st.tabs([
    ...
    with tabs[0]:
        ...
        v_bear, _, _, _ = ValuationModel.run_dcf_pro(data['fcf_now_b'], g1_in*0.5, 0.02, final_wacc+0.02, macro_adj=-0.15, shares=data['shares_m'], cash=data['cash_b'], debt=data['debt_b'])
        v_bull, _, _, _ = ValuationModel.run_dcf_pro(data['fcf_now_b'], g1_in+0.05, 0.12, final_wacc-0.01, macro_adj=0.10, shares=data['shares_m'], cash=data['cash_b'], debt=data['debt_b'])
        ...
        fig_water.update_layout(title="Composición del Valor Institucional (Billones USD)", template="plotly_dark", height=450)
        st.plotly_chart(fig_water, use_container_width=True)

    # --- TAB 2: SCORECARD & RADAR (IA INSIGHTS) ---
    with tabs[1]:
        ...
        with col_d1:
            ...
            diags = [
                ...
                ("Retención de membresía >90% (SEC 10-K)", True, "star"),
                ("Calidad de ganancias superior al promedio", True, "star")
            ]
    ...
    with tabs[2]:
        ...
        with r_col2:
            st.write("Historial de EPS Notificado vs Pronóstico")
    ...
    # --- TAB 5: STRESS TEST PRO (MATRIZ BLACK SWAN) ---
    with tabs[4]:
        ...
        # MODELO MACRO CORRELACIONADO
        v_stress, _, _, _ = ValuationModel.run_dcf_pro(
            data['fcf_now_b'] * (1 + sw_imp), 
            g1_in, g2_in, final_wacc + wacc_shock, 
            shares=data['shares_m'], cash=data['cash_b'], debt=data['debt_b'],
            macro_adj=macro_fcf_impact
        )
    ...
    # --- TAB 7: DCF LAB (CONTINUIDAD DE TENDENCIA) ---
    with tabs[6]:
        ...
        fig_dcf.update_layout(title="Bridge de Generación de Caja: Histórico SEC vs Proyección Oracle ($B)", template="plotly_dark", height=550)
        ...
        mtx = [[ValuationModel.run_dcf_pro(data['fcf_now_b'], g1_in, g2_in, w, g, shares=data['shares_m'], cash=data['cash_b'], debt=data['debt_b'], macro_adj=macro_fcf_impact)[0] for g in g_rng] for w in w_rng]
    ...
    # --- TAB 4, 8, 10: CONTINUIDAD DE FUNCIONALIDAD ---
    # Se mantienen por integridad del archivo extendido (Forward, Monte Carlo, Opciones)
    with tabs[3]:
        st.subheader("Forward Looking Results")
        fwd_rev = data['info'].get('totalRevenue', 250e9)
        yrs = [2026, 2027, 2028, 2029, 2030]
        df_fwd = pd.DataFrame({"Año": yrs, "Est. Rev ($B)": [fwd_rev*(1.08)**i/1e9 for i in range(1,6)]})
        st.table(df_fwd.style.format("{:.2f}"))
    with tabs[7]:
        st.subheader("Simulación Monte Carlo")
        sims = [ValuationModel.run_dcf_pro(data['fcf_now_b'], np.random.normal(g1_in, 0.05), g2_in, np.random.normal(final_wacc, 0.005), macro_adj=macro_fcf_impact, shares=data['shares_m'], cash=data['cash_b'], debt=data['debt_b'])[0] for _ in range(300)]
        st.plotly_chart(px.histogram(sims, title="Probabilidad de Fair Value", color_discrete_sequence=['#005BAA']), use_container_width=True)
    with tabs[9]:
        st.subheader("Laboratorio de Griegas")
        grks = ValuationModel.calculate_greeks(p_ref, p_ref*1.05, 45/365, 0.045, 0.25)
        st.metric("Call Price Est.", f"${grks['price']:.2f}")
...
# --- FIN DEL DOCUMENTO MASTER v18.0 (1100+ LÍNEAS LÓGICAS) ---
