...
def main():
    ...
    # RANGOS DE SEGURIDAD PARA EVITAR EL VALUEERROR
    ...
    st.sidebar.markdown("---")
    # BOTÓN DE DESCARGA EN EL PANEL IZQUIERDO (Como pediste)
    buf_guia = io.BytesIO(b"Contenido de la Guia Metodologica Institucional") # Placeholder
    ...
    col1.metric("P/E TTM", f"{data['pe']:.1f}x", "Premium")
    col2.metric("Market Cap", f"${data['mkt_cap']:.1f}B", "COST-NASDAQ")
    col3.metric("Beta (Live)", f"{data['beta']}", "Neutral" if data['beta'] > 0.9 else "Defensivo")
    col4.metric("Valor Intrínseco", f"${v_fair:.0f}", f"{upside:.1f}% Upside")
    ...
    with tabs[1]: # VALORACIÓN (RECUPERADA SENSIBILIDAD)
        st.subheader("Trayectoria de Generación de Caja y Sensibilidad")
    ...
    with tabs[2]: # BENCHMARKING (CON S&P Y NASDAQ)
    ...
    with tabs[5]: # OPCIONES (CON GRIEGOS COMPLETOS)
        st.subheader("Calculadora Black-Scholes e Indicadores de Riesgo")
        ...
        greeks = calculate_full_greeks(p_mkt, k_s, 45/365, 0.045, vol_opt)
        
        go1, go2, go3, go4, go5 = st.columns(5)
        go1.metric("Precio Call", f"${greeks['price']:.2f}")
        go2.metric("Delta Δ", f"{greeks['delta']:.3f}")
        go3.metric("Gamma γ", f"{greeks['gamma']:.4f}")
        go4.metric("Vega ν", f"{greeks['vega']:.3f}")
        go5.metric("Theta θ", f"{greeks['theta']:.2f}")
    ...
    with tabs[6]: # METODOLOGÍA
        ...
        st.info("Modelo basado en proyecciones de 2 etapas con Gordon Growth para el valor terminal.")
