...
def main():
    ...
    payload = data_service.fetch_verified_payload("COST")

    if payload is None:
        st.error("No se pudo inicializar el motor de datos.")
        return
...
# -------------------------------------------------------------------------
    # TAB 3: PEER ANALYSIS & MARKET BENCHMARKING (ESTILO PORTAL + INTERACTIVO)
    # -------------------------------------------------------------------------
    with tabs[2]:
        ...
        # 2. SELECTOR ON-DEMAND
        selected_labels = st.multiselect(
            "Selecciona activos y benchmarks para el análisis comparativo:",
            options=list(market_map.keys()),
            default=[
                "S&P 500 (Market)", "Nasdaq 100 (Tech)", "Walmart (WMT)", 
                "Target (TGT)", "Amazon (AMZN)", "Pricesmart (PSMT)", 
                "BJ's Wholesale (BJ)", "Kroger (KR)", "Home Depot (HD)", 
                "Lowe's (LOW)", "Sprouts (SFM)"
            ],
            help="Agregue índices o competidores para recalcular la terminal."
        )
        ...
        with st.spinner("Sincronizando terminal con Wall Street..."):
            df_full_comparison = data_service.fetch_peer_group_data(full_ticker_list)

        # 3. BLOQUE DE SEGURIDAD Y NORMALIZACIÓN
        bunker_names = {
        ...
        if df_full_comparison is not None and not df_full_comparison.empty:
            ...
            if "COST" not in df_full_comparison['Ticker'].values:
                cost_row = pd.DataFrame([{
                    ...
                    "EV/EBITDA": 33.0, "Net Margin (%)": 2.6, "Rev Growth (%)": 6.0
                }])
        ...
        with a1:
            st.markdown('<p class="scenario-label-sober">📊 VALORACIÓN</p>', unsafe_allow_html=True)
            st.table(pd.DataFrame({
                "Ratio": ["P/E Ratio", "EV/EBITDA", "Mkt Cap"],
                "COST": [f"{c_stats.get('P/E Ratio', 0):.1f}x", f"{c_stats.get('EV/EBITDA', 0):.1f}x", f"${c_stats.get('Mkt Cap ($B)', 0):.1f}B"],
                "Industry": [f"{ind_avg.get('P/E Ratio', 0):.1f}x", f"{ind_avg.get('EV/EBITDA', 0):.1f}x", f"${ind_avg.get('Mkt Cap ($B)', 0):.1f}B"]
            }))
        with a2:
            st.markdown('<p class="scenario-label-sober">📈 RENTABILIDAD</p>', unsafe_allow_html=True)
            st.table(pd.DataFrame({
                "Ratio": ["ROE (%)", "Net Margin", "Rev Growth"],
                "COST": [f"{c_stats.get('ROE (%)', 0):.1f}%", f"{c_stats.get('Net Margin (%)', 0):.1f}%", f"{c_stats.get('Rev Growth (%)', 0):.1f}%"],
                "Industry": [f"{ind_avg.get('ROE (%)', 0):.1f}%", f"{ind_avg.get('Net Margin (%)', 0):.1f}%", f"{ind_avg.get('Rev Growth (%)', 0):.1f}%"]
            }))
        with a3:
            st.markdown('<p class="scenario-label-sober">🛡️ SOLVENCIA</p>', unsafe_allow_html=True)
            st.table(pd.DataFrame({
                "Ratio": ["Current Ratio", "Debt/Equity", "Beta"],
                "COST": [f"{payload['acc_summary'].get('Current Ratio', 0):.2f}x", f"{payload['acc_summary'].get('Debt/Equity', 0):.1f}%", f"{payload.get('beta', 0):.2f}"],
                "Industry": [f"{ind_avg.get('Current Ratio', 1.0):.2f}x", f"{ind_avg.get('Debt/Equity', 40):.1f}%", "1.00"]
            }))
        ...
        with c_p1:
            st.write("**P/E vs ROE**")
            fig_scat = px.scatter(df_full_comparison, x="P/E Ratio", y="ROE (%)", size="Mkt Cap ($B)", 
                                 color="Nombre", text="Ticker", template="plotly_dark")
            fig_scat.update_yaxes(range=[-5, 75])
            fig_scat.update_layout(height=400, showlegend=False)
            st.plotly_chart(fig_scat, use_container_width=True)
        
        with c_p2:
            st.write("**Múltiplos EV/EBITDA**")
            df_bar = df_full_comparison.sort_values("EV/EBITDA")
            fig_bar = px.bar(df_bar, x="Ticker", y="EV/EBITDA", color="Nombre", template="plotly_dark")
            fig_bar.update_layout(height=400, showlegend=False)
            st.plotly_chart(fig_bar, use_container_width=True)

        # --- CORRELACIÓN Y TABLA MAESTRA ---
        st.markdown("---")
        ...
        with col_m2:
            ...
            df_master = df_full_comparison.copy().rename(columns={
                "Mkt Cap ($B)": "Mkt Cap ($)", "P/E Ratio": "P/E (x)", "EV/EBITDA": "EV/EBIT (x)"
            }).set_index("Ticker")
            st.dataframe(
                df_master.style.format({
                    "Mkt Cap ($)": "${:.1f}B", "P/E (x)": "{:.1f}x", "ROE (%)": "{:.1f}%"
                }).background_gradient(cmap="Greens", subset=["ROE (%)"])
                .background_gradient(cmap="Reds", subset=["P/E (x)"]),
