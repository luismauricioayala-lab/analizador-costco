...
# -------------------------------------------------------------------------
    # TAB 3: PEER ANALYSIS & MARKET BENCHMARKING (PORTAL STYLE & DEEP DIVE)
    # -------------------------------------------------------------------------
    with tabs[2]:
        st.subheader("🔬 Peer Analysis & Institutional Benchmarking")
        
        # 1. MAPEOS Y SELECTORES
        market_map = {
            "S&P 500 (Market)": "SPY",
            "Nasdaq 100 (Tech)": "QQQ",
            "Walmart (WMT)": "WMT",
        ...
        selected_labels = st.multiselect(
            "Seleccione activos para benchmark competitivo:",
            options=list(market_map.keys()),
            default=["S&P 500 (Market)", "Walmart (WMT)", "Target (TGT)", "Pricesmart (PSMT)", "BJ's Wholesale (BJ)"]
        )
        ...
        with st.spinner("Sincronizando Terminal..."):
            df_peers = data_service.fetch_peer_group_data(full_ticker_list)

        if df_peers is not None and not df_peers.empty:
            # --- LIMPIEZA Y NORMALIZACIÓN ---
            df_peers = df_peers.drop_duplicates(subset=['Ticker'])
            bunker_names = {"COST": "Costco", "PSMT": "Pricesmart", "SPY": "S&P 500", "QQQ": "Nasdaq 100"}
            df_peers['Nombre'] = df_peers['Ticker'].map(bunker_names).fillna(df_peers['Nombre'])

            # =========================================================================
            # BLOQUE PORTAL: AUDITORÍA DE RATIOS (Categorizados)
            # =========================================================================
            st.markdown("### 🏛️ Módulo de Auditoría: Benchmarking de Calidad")
            
            # Calculamos promedios de la industria (excluyendo a COST)
            df_industry = df_peers[df_peers['Ticker'] != 'COST']
            industry_avg = df_industry.mean(numeric_only=True)
            cost_stats = df_peers[df_peers['Ticker'] == 'COST'].iloc[0]

            # Estilo Portal: Tres columnas de Ratios
            col_a, col_b, col_c = st.columns(3)

            with col_a:
                st.markdown('<p class="scenario-label-sober">📊 VALORACIÓN (Multiples)</p>', unsafe_allow_html=True)
                val_data = {
                    "Métrica": ["P/E Ratio", "EV/EBITDA", "P/Sales"],
                    "Costco": [f"{cost_stats.get('P/E Ratio', 0):.1f}x", f"{cost_stats.get('EV/EBITDA', 0):.1f}x", f"{cost_stats.get('Price / Revenue', 0):.2f}x"],
                    "Ind. Avg": [f"{industry_avg.get('P/E Ratio', 0):.1f}x", f"{industry_avg.get('EV/EBITDA', 0):.1f}x", f"{industry_avg.get('Price / Revenue', 0):.2f}x"]
                }
                st.table(pd.DataFrame(val_data))

            with col_b:
                st.markdown('<p class="scenario-label-sober">📈 RENTABILIDAD (Returns)</p>', unsafe_allow_html=True)
                prof_data = {
                    "Métrica": ["ROE (%)", "Net Margin", "ROA (%)"],
                    "Costco": [f"{cost_stats.get('ROE (%)', 0):.1f}%", f"{cost_stats.get('Net Margin (%)', 0):.1f}%", f"{cost_stats.get('ROA (%)', 0):.1f}%"],
                    "Ind. Avg": [f"{industry_avg.get('ROE (%)', 0):.1f}%", f"{industry_avg.get('Net Margin (%)', 0):.1f}%", f"{industry_avg.get('ROA (%)', 0):.1f}%"]
                }
                st.table(pd.DataFrame(prof_data))

            with col_c:
                st.markdown('<p class="scenario-label-sober">🛡️ SOLVENCIA (Risk)</p>', unsafe_allow_html=True)
                risk_data = {
                    "Métrica": ["Current Ratio", "Debt/Equity", "Beta"],
                    "Costco": [f"{cost_stats.get('Current Ratio', 0):.2f}x", f"{cost_stats.get('Debt/Equity', 0):.1f}%", f"{payload.get('beta', 0):.2f}"],
                    "Ind. Avg": [f"{industry_avg.get('Current Ratio', 0):.2f}x", f"{industry_avg.get('Debt/Equity', 0):.1f}%", "1.00"]
                }
            ...
            # 1. RENDIMIENTO NORMALIZADO (BASE 100)
            st.write("**Rendimiento Relativo 1Y (Normalizado Base 100)**")
            if os.path.exists("market_history.csv"):
                perf_df = pd.read_csv("market_history.csv", index_col=0, parse_dates=True)
                # Filtrar solo tickers seleccionados + COST
                active_tickers = [t for t in full_ticker_list if t in perf_df.columns]
                if active_tickers:
                    perf_norm = (perf_df[active_tickers] / perf_df[active_tickers].iloc[0]) * 100
                    fig_line = px.line(perf_norm, template="plotly_dark")
                    fig_line.update_traces(line=dict(width=1), opacity=0.5)
                    if "COST" in perf_norm.columns:
                        fig_line.update_traces(selector=dict(name="COST"), line=dict(width=4, color="#005BAA"), opacity=1)
                    fig_line.update_layout(height=400, margin=dict(l=0,r=0,t=0,b=0), legend=dict(orientation="h", y=-0.2))
                    st.plotly_chart(fig_line, use_container_width=True)

            # 2. DISPERSIÓN VALORACIÓN VS RENTABILIDAD
            c_g1, c_g2 = st.columns(2)
            with c_g1:
                st.write("**Eficiencia vs Valoración (P/E vs ROE)**")
                fig_scat = px.scatter(df_peers, x="P/E Ratio", y="ROE (%)", size="Mkt Cap ($B)", 
                                     color="Ticker", text="Ticker", template="plotly_dark")
                fig_scat.update_layout(height=400, showlegend=False)
                st.plotly_chart(fig_scat, use_container_width=True)
            
            with c_g2:
                st.write("**Márgenes Netos Comparativos**")
                df_bar = df_peers.sort_values("Net Margin (%)")
                fig_bar = px.bar(df_bar, x="Ticker", y="Net Margin (%)", color="Ticker", template="plotly_dark")
                fig_bar.update_layout(height=400, showlegend=False)
                st.plotly_chart(fig_bar, use_container_width=True)

            # =========================================================================
            # MATRIZ DE CORRELACIÓN Y TABLA MAESTRA
            # =========================================================================
            st.markdown("---")
            col_m1, col_m2 = st.columns([1, 2])
            
            with col_m1:
                st.write("**🧩 Correlación (Daily Returns)**")
                if 'perf_df' in locals():
                    corr = perf_df[active_tickers].pct_change().corr()
                    fig_corr = px.imshow(corr, text_auto=".2f", color_continuous_scale='RdBu_r', template="plotly_dark")
                    fig_corr.update_layout(height=350, margin=dict(l=0,r=0,t=0,b=0))
                    st.plotly_chart(fig_corr, use_container_width=True)

            with col_m2:
                st.write("**📑 Matriz de Auditoría Final**")
                # Formateo institucional
                df_fmt = df_peers.copy().set_index("Ticker")
                st.dataframe(
                    df_fmt.style.format({
                        "Mkt Cap ($B)": "${:.1f}B",
                        "P/E Ratio": "{:.1f}x",
                        "ROE (%)": "{:.1f}%",
                        "Net Margin (%)": "{:.2f}%"
                    }).background_gradient(cmap="Greens", subset=["ROE (%)"])
                    .background_gradient(cmap="Reds", subset=["P/E Ratio"]),
                    use_container_width=True, height=350
                )
        else:
            st.error("No se pudieron cargar los datos de Peers. Verifique la conexión o el Búnker.")
